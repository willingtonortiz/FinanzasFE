...
export class RecordComponent implements OnInit {
	...
	public historial: Map<string, any>;
	...
	async ngOnInit() {
		this.records = await this._recordService.findByUserId(
			this.currentUser.id
		);
		...
		this.records.forEach(r => {
			let date: string =
				r.date.getDate() +
				" de " +
				this.NameMonth(r.date.getMonth()) +
				" de " +
				r.date.getUTCFullYear();

			let h: string;
			if (r.date.getHours() < 5) h = (r.date.getHours() + 19).toString();
			else if (r.date.getHours() < 15)
				h = "0" + (r.date.getHours() - 5).toString();
			else h = (r.date.getHours() - 5).toString();
			let m: string;
			if (r.date.getMinutes() < 10) m = "0" + r.date.getMinutes();
			else m = r.date.getMinutes().toString();
			let time: string = h + ":" + m;
